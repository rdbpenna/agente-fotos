"""
GUI PySide6 — v25: visual polish for titlebar, bracketing spacing, labels and preview loading.
"""
...
from PySide6.QtGui import (
    ...
    QIcon, QPainterPath, QLinearGradient, QPalette, QImageReader,
)
...
QLabel {{ color: {T['text1']}; background-color: transparent; border: none; }}
QFrame#Card QLabel, QFrame#SubCard QLabel {{ background-color: transparent; border: none; }}
...
class SegGroup(QWidget):
    """Segmented control com largura calculada para não cortar textos."""
    changed = Signal(str)
    def __init__(self, options, current="", parent=None, min_button_width=74, equal=True):
        super().__init__(parent)
        self._btns = {}
        h = QHBoxLayout(self)
...
class PhotoAgentApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self._bracketing_group_size = "auto"
        self._bracketing_fusion_preset = "natural"
        self._build_ui()
        self._apply_windows_dark_titlebar()

    def _apply_windows_dark_titlebar(self):
        """Deixa a barra nativa do Windows escura para combinar com o mockup."""
        ...
        try:
            ...
            # Windows 11/10 20H1+: 20; builds antigas: 19
            for attr in (20, 19):
    ...
    def _build_ui(self):
        ...
        ll.addWidget(self._mk_bracketing())
    ...
    def _mk_pastas(self):
        ...
        for label, desc, attr in [
            ...
            b.clicked.connect(lambda _, w=le, is_input=(attr == "_input_dir"): self._browse_dir(w, is_input)); row.addWidget(b)
    ...
    # ── Bracketing / HDR ──────────────────────────────────────────
    def _mk_bracketing(self):
        card = Card()
        self._hdr(card, "Bracketing / HDR", "Fundir 3 ou 5 exposições do mesmo enquadramento antes do enhance.",
                  icon=Icons.activity(T['text2']))

        top = QHBoxLayout(); top.setSpacing(12)
        self._tg_bracket = Toggle(False)
        self._tg_bracket.toggled_signal.connect(self._on_bracketing)
        top.addWidget(self._tg_bracket, 0, Qt.AlignVCenter)
        lbl = QLabel("Ativar modo bracketing")
        lbl.setStyleSheet(f"font-size:13px;font-weight:500;color:{T['text1']};")
        top.addWidget(lbl, 1)
        card.l(top)
        card.sp(12)

        # Grupo em linha própria para evitar corte de texto
        l1 = QLabel("Grupo")
        l1.setStyleSheet(f"font-size:12px;font-weight:600;color:{T['text1']};")
        card.w(l1)
        self._seg_bracket_group = SegGroup(["Auto", "3 fotos", "5 fotos"], "Auto", min_button_width=82, equal=True)
        self._seg_bracket_group.changed.connect(self._on_bracket_group)
        card.w(self._seg_bracket_group)
        card.sp(10)

        # Fusão em linha própria; quatro opções não cabem bem lado a lado com o grupo
        l2 = QLabel("Fusão")
        l2.setStyleSheet(f"font-size:12px;font-weight:600;color:{T['text1']};")
        card.w(l2)
        self._seg_bracket_preset = SegGroup(["Natural", "Janela", "Interior", "Luxury"], "Natural", min_button_width=86, equal=True)
        self._seg_bracket_preset.changed.connect(self._on_bracket_preset)
        card.w(self._seg_bracket_preset)
        card.sp(8)

        help_txt = QLabel("Auto detecta sequências por horário/EXIF, resolução e variação de exposição.")
        help_txt.setWordWrap(True)
        help_txt.setStyleSheet(f"font-size:11.5px;color:{T['text4']};")
        card.w(help_txt)
        return card
    ...
    #  HANDLERS
    # ══════════════════════════════════════════════════════════════
    def _browse_dir(self, le, is_input=False):
        p = QFileDialog.getExistingDirectory(self, "Selecionar pasta")
        if p:
            le.setText(p)
            if is_input:
                self._load_preview_from_dir(p, show_feedback=True)
    ...
    def _on_bracketing(self, v): self._bracketing_enabled = v
    def _on_bracket_group(self, v):
        self._bracketing_group_size = {"Auto": "auto", "3 fotos": "3", "5 fotos": "5"}.get(v, "auto")
    def _on_bracket_preset(self, v):
    ...
    def _find_preview_images(self, folder):
        """Encontra imagens no diretório; se não achar, tenta uma busca recursiva rasa."""
        if not folder or not os.path.isdir(folder):
            return []
        exts = (".jpg", ".jpeg", ".png", ".webp", ".bmp", ".tiff", ".tif")
        images = []
        # Primeiro: somente arquivos diretos da pasta selecionada
        try:
            ...
            return []
        # Fallback: subpastas, limitado para não travar em diretórios enormes
        if not images:
            for root, dirs, files in os.walk(folder):
                if root != folder and root.count(os.sep) - folder.count(os.sep) > 1:
                    dirs[:] = []
                    continue
                for name in files:
                ...
                if len(images) >= 50:
                    break
        images.sort(key=lambda x: (os.path.dirname(x), os.path.basename(x).lower()))
        return images

    def _load_pixmap_autotransform(self, path):
        """Carrega imagem respeitando orientação EXIF quando possível."""
    ...
    def _load_preview_from_dir(self, path, show_feedback=False):
        images = self._find_preview_images(path)
        if not images:
            self._sb_ct.setText("0 fotos na fila")
            if show_feedback:
                self._sb_st.setText("Nenhuma imagem encontrada para preview")
            return
        first_image = images[0]
        before_px = self._load_pixmap_autotransform(first_image)
        if before_px.isNull():
        ...
        self._sb_st.setText(f"Preview: {os.path.basename(first_image)}")

    def _on_input_dir_changed(self, path):
        """Auto-load first image from input folder into preview."""
        self._load_preview_from_dir(path, show_feedback=False)
